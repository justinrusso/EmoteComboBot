...
def get_channel_id(channel_name: str, config: dict[str, any]) -> str:
    headers = { 'Authorization': f'Bearer {config.oauth}', 'Client-Id': config.client_id }
...
def main():
    ...
    channel_id = get_channel_id(config['channel'], {"oauth": config['accessToken'], "client_id": config['clientId']})
    emotes = get_emotes(config['channel'], channel_id)

    client = ComboBot(config['botName'], f'oauth:{config["accessToken"]}').start()
